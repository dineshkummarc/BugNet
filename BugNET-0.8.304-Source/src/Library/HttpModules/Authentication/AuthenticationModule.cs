...
namespace BugNET.HttpModules
{
    ...
    public class AuthenticationModule : IHttpModule
    {
        ...
        void context_AuthenticateRequest(object sender, EventArgs e)
        {
            ...
            // You might be able to spoof the path and bypass later checks which are 
            // expecting to be given an exception if there is a problem.
            //
            // However this is a nice solution.
                        ...
                        if (user == null)
                        {
                            try
                            {
                                ...
                                if (createStatus == MembershipCreateStatus.Success && mu != null)
                                {
                                    ...
                                    List<Role> roles = Role.GetAllRoles().FindAll(r => r.AutoAssign == true);
        ...
        public UserProperties GetUserProperties(string identification)
        {
            ...
            else if (Globals.UserAccountSource == "ActiveDirectory")
            {
                ...
                if (string.IsNullOrEmpty(HostSetting.GetHostSetting("ADUserName")))
                    entry = new DirectoryEntry(String.Format("{0}{1}", "GC://", HostSetting.GetHostSetting("ADPath")), null, null, AuthenticationTypes.ReadonlyServer);
                else
                    entry = new DirectoryEntry(String.Format("{0}{1}", "GC://", HostSetting.GetHostSetting("ADPath")), HostSetting.GetHostSetting("ADUserName"), HostSetting.GetHostSetting("ADPassword"), AuthenticationTypes.Secure);
