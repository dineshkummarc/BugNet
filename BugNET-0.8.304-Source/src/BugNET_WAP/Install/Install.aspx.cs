...
namespace BugNET.Install
{
    public partial class Install : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    switch (Upgrade.GetUpgradeStatus())
                    {
        ...
        {
            WriteHeader("none");
            WriteMessage(string.Format("<h2>Current Database Version: {0}</h2>", Upgrade.GetInstalledVersion()));
            WriteMessage(string.Format("<h2>Current Assembly Version: {0}</h2>", Upgrade.GetCurrentVersion()));
        ...
        private void InstallApplication()
        {
            ...
            if (installationDate == null || installationDate == "")
            {
                ...
                //update machine key.
                string error = Upgrade.UpdateMachineKey();
            ...
            {              
                ...
                WriteMessage(string.Format("<h2>Version: {0}</h2>", Upgrade.GetCurrentVersion()));
        ...
        private bool InstallBugNET()
        {
            try
            {
                string providerPath = DataProviderManager.Provider.GetProviderPath();            

                if (!providerPath.StartsWith("ERROR"))
                {
                    WriteMessage(string.Format("Installing Version: {0}<br/>", Upgrade.GetCurrentVersion()), 0, true);
                    ...
                    Upgrade.UpdateDatabaseVersion(Upgrade.GetCurrentVersion());
                }
                else
                {
                    //upgrade error
        ...
        private void UpgradeApplication()
        {
            ...
            else
            {
                ...
                WriteMessage(string.Format("<h2>Current Assembly Version: {0}</h2>", Upgrade.GetCurrentVersion()));
                WriteMessage(string.Format("<h2>Current Database Version: {0}</h2>", Upgrade.GetInstalledVersion()));
                WriteMessage(string.Format("Upgrading To Version: {0}<br/>", Upgrade.GetCurrentVersion()), 0, true);
                if (UpgradeBugNET())
                {
        ...
        private bool UpgradeBugNET()
        {
            try
            {
                  ...
                  if (!providerPath.StartsWith("ERROR"))
                  {
                      ...
                      int AssemblyVersion = Convert.ToInt32(Upgrade.GetCurrentVersion().Replace(".", ""));
                      int DatabaseVersion = Convert.ToInt32(Upgrade.GetInstalledVersion().Replace(".", ""));
