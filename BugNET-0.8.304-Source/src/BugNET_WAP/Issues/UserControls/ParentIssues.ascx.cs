namespace BugNET.Issues.UserControls
{
	...
	public partial class ParentIssues : System.Web.UI.UserControl, IIssueTab
	{
        ...
        public void Initialize()
        {
            ...
            if (!Page.User.Identity.IsAuthenticated || !ITUser.HasPermission(ProjectId, Globals.Permission.DELETE_PARENT_ISSUE.ToString()))
                IssuesDataGrid.Columns[4].Visible = false;

            //check users role permission for adding a related issue
            if (!Page.User.Identity.IsAuthenticated || !ITUser.HasPermission(ProjectId, Globals.Permission.ADD_PARENT_ISSUE.ToString()))
