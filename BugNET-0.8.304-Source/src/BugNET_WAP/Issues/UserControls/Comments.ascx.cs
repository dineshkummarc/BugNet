namespace BugNET.Issues.UserControls
{
	...
	public partial class Comments : System.Web.UI.UserControl, IIssueTab
	{
		...
		private void BindComments()
		{
			...
			{
				_IssueOwnerUserId = Issue.GetIssueById(IssueId).OwnerUserId;
		...
		protected void rptComments_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				...
				if (HostSetting.GetHostSetting("EnableGravatar", true))
				{
					MembershipUser user = Membership.GetUser(currentComment.CreatorUserName);
                    Avatar.Attributes.Add("src", GetGravatarImageURL((user == null) ? string.Empty /* user profile not upgraded? */ : user.Email, 35));
				}
				else
				{
                ...
                else if ((Page.User.Identity.IsAuthenticated && ITUser.IsInRole(Globals.SuperUserRole)) || (Page.User.Identity.IsAuthenticated && ITUser.IsInRole(this.ProjectId, Globals.ProjectAdminRole)))
                    lnkEditComment.Visible = true;
                // Check if it is the original user, the project admin or a super user trying to edit the comment.
                else if (currentComment.CreatorUserName.ToLower() == Context.User.Identity.Name.ToLower() && ITUser.HasPermission(ProjectId, Globals.Permission.OWNER_EDIT_COMMENT.ToString()))
                    lnkEditComment.Visible = true;
