    ...
    public class Project
    {
                ...
                if (TempId > Globals.NewId)
                {
                    ...
                    if (this.AttachmentStorageType == IssueAttachmentStorageType.FileSystem)
                    {
                        try
                        {
                            ...
                            if (!checkUploadPath(@"~\Uploads\" + _UploadPath))
        ...
        private bool UpdateProject()
        {
            ...
            if (this.AttachmentStorageType == IssueAttachmentStorageType.FileSystem && p.UploadPath != UploadPath)
            {
                ...
                string newPath = @"~" + Globals.UploadFolder + _UploadPath.Trim();
                ...
                if (isPathNorty)
                {                    
                    if (Log.IsErrorEnabled) Log.Error(string.Format(Logging.GetErrorMessageResource("CouldNotCreateUploadDirectory"), HttpContext.Current.Server.MapPath(@"~\Uploads\" + _UploadPath)));
                    return false;
                }
        ...
        public static bool checkUploadPath(string sPath)
        {
            ...
            int tmpArbLength = 64;
            if ((tmpPath.Length > tmpArbLength))
            {
                isPathNorty = true;
            }
