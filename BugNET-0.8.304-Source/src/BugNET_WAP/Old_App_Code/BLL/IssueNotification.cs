...
namespace BugNET.BusinessLogicLayer
{
    ...
    public class IssueNotification
    {
        ...
        public static void SendIssueNotifications(int issueId)
        {
            ...
            //TODO: Move this internal to the notification manager ??
            //pass in a notification type parameter to decide which notification to send and the issue.
            ...
            string template = nm.LoadEmailNotificationTemplate("IssueUpdated");
            ...
            foreach (IssueNotification notify in issNotifications)
            {
                try
                {
                    ...
                    {
                        string notificationUserDisplayName = ITUser.GetUserDisplayName(notify.NotificationDisplayName);
                        nm.SendNotification(notify.NotificationUsername, String.Format(subject, issue.FullId, displayname), template, notificationUserDisplayName);
                    } 
                }
                catch (Exception ex)
                {
        ...
        public static void SendIssueAddNotifications(int issueId)
        {
            ...
            //load template and replace the tokens
            string template = nm.LoadEmailNotificationTemplate("IssueAdded");
            Dictionary<string, object> data = new Dictionary<string, object>();
            ...
            foreach (IssueNotification notify in issNotifications)
            {
                try
                {
                    ...
                    //if (notify.NotificationUsername != Security.GetUserName())
                    string notificationUserDisplayName = ITUser.GetUserDisplayName(notify.NotificationDisplayName);
                    nm.SendNotification(notify.NotificationUsername, String.Format(subject, issue.FullId, issue.ProjectName), template, notificationUserDisplayName);
                }
                catch (Exception ex)
                {
                    ProcessException(ex);
        ...
        public static void SendIssueNotifications(int issueId, List<IssueHistory> issueChanges)
        {
            ...
            //load template and replace the tokens
            string template = nm.LoadEmailNotificationTemplate("IssueUpdatedWithChanges");
            Dictionary<string, object> data = new Dictionary<string, object>();
            ...
            foreach (IssueNotification notify in issNotifications)
            {
                try
                {
                    ...
                    //if (notify.NotificationUsername != Security.GetUserName())
                    string notificationUserDisplayName = ITUser.GetUserDisplayName(notify.NotificationDisplayName);
                    nm.SendNotification(notify.NotificationUsername, String.Format(subject, issue.FullId, displayname), String.Format(template, issueChanges), notificationUserDisplayName);
                }
                catch (Exception ex)
                {
                    ProcessException(ex);
        ...
        public static void SendNewAssigneeNotification(int issueId, string newAssigneeUserName)
        {
            ...
            //load template and replace the tokens
            string template = nm.LoadEmailNotificationTemplate("NewAssignee");
            Dictionary<string, object> data = new Dictionary<string, object>();
            ...
            string displayname = ITUser.GetUserDisplayName(Security.GetUserName());
            string notificationUserDisplayName = ITUser.GetUserDisplayName(newAssigneeUserName);

            try
            {
                //send notifications to the new assignee
                nm.SendNotification(newAssigneeUserName, String.Format(subject, issue.FullId), template, notificationUserDisplayName);
            }
            catch (Exception ex)
            {
                ProcessException(ex);
