...
namespace BugNET.Administration.Projects
{
	...
	public partial class EditProject :  BugNET.UserInterfaceLayer.BasePage 
	{
		...
		{
            if (!ITUser.HasPermission(Convert.ToInt32(Request.QueryString["id"]), Globals.Permission.ADMIN_EDIT_PROJECT.ToString()))
			...
			if (!Page.IsPostBack)
			{
                ...
                if (!ITUser.HasPermission(Convert.ToInt32(Request.QueryString["id"]), Globals.Permission.ADMIN_DELETE_PROJECT.ToString()))
                {
